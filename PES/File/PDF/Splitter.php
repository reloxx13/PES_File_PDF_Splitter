...
class PES_File_PDF_Splitter {
	...
	public function convertToIMGs($destination, $qulity = 100, $page = array(), $size_x = 0, $size_y = 0) {
		if (empty($page)) {
			return;
		}
		//		$this->setXResolution(180);
		...
		if (!is_dir($destination) or !is_writeable($destination)) {
			...
			foreach ($pdf as $index => $a_page) {
				...
				$a_page->setImageCompressionQuality($qulity);
				$a_page->setImageFormat('jpeg');
				$a_page->setImageAlphaChannel(Imagick::ALPHACHANNEL_REMOVE);
				if ($size_x && $size_y) {
					//	$a_page->thumbnailImage($size_x, $size_y, TRUE, TRUE);
				}
